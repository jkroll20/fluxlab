    ...
    switch(event)
    {
        ...
        case MOUSE_OVER:
            if(moving==self->id)
            {
                ...
                if(x<0) x= 0;
                else if(x>prc.rgt) x= prc.rgt;
                if(y<0) y= 0;
                else if(y>prc.btm) y= prc.btm;
...
prop_t lblist_cbprops(prop_t arg, primitive *self, int type, const char *name, int id, prop_t value)
{
    ...
    if(!data) {
        logmsg("lblist_cbprops: self->privdata==NULL\n");
        exit(1);
    }
    ...
    switch(type)
    {
        ...
        case PROP_SET:
            ...
            else if(!strcmp(name, "item_text") || !strcmp(name, "item_data"))
            {
                ...
                int i= data->index;
                if(i<0) i= 0;
                else if(i>data->n_items-1) i= data->n_items-1;
...
int icon_cbmouse(prop_t arg, primitive *self, int event, int x, int y, int btn)
{
    ...
    switch(event)
    {
        case MOUSE_DOWN:
            ...
            move_start.x= mouse_x();
            move_start.y= mouse_y();
            wndpos_start.x= self->parent->x;
            wndpos_start.y= self->parent->y;
            moving= self->id;
            wnd_set_mouse_capture(self->id);
        ...
        case MOUSE_UP:
            {
                ...
                int xa= 8, ya= 8;
                if(self->parent->x<0) xa= -8;
                if(self->parent->y<0) ya= -8;
